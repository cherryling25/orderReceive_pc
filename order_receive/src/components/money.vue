    ...
    >
      ...
      <el-main>
        ...
        >
          ...
          <el-table-column prop="renwu_id" label="任务编号"> </el-table-column>

          <el-table-column prop="time" label="发布时间">
          </el-table-column>
          <el-table-column prop="price" label="佣金价格">
          </el-table-column>

          <el-table-column prop="" label="状态">
            <template slot-scope="scope">
              <div v-if="scope.row.state==1">正常</div>
               <div v-if="scope.row.state==2">冻结资金</div>
            </template>
          </el-table-column>

          <el-table-column prop="centons" label="备注">
          </el-table-column>
          <el-table-column prop="order_price" label="当前价格">
          </el-table-column>
  <el-table-column prop="state" label="状态">
            <template slot-scope="scope">
              <div v-if="scope.row.type==1">未结账</div>
               <div v-if="scope.row.type==2">已结账</div>
            </template>
          </el-table-column>
  ...
  methods: {
    ...
    showData(page,size) {
      axios({
        ...
        url: this.GLOBAL.hostUrl5 + "hands/Order/price_list"
      }).then(res => {
        console.log(res);
